extends Area2D
## Lightweight collision zone for gas trail chemistry reactions
## No visuals - just collision detection for ChemistryManager
...
func initialize(data: AttackData, pos: Vector2, duration: float, attack_owner: Node2D = null) -> void:
	...
	# Configure larger collision radius (2.5x original for sparse zones)
	var shape = $CollisionShape2D.shape as CircleShape2D
	if shape:
		shape.radius = data.collision_radius * 2.5
